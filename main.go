...
func processDomain(ctx context.Context, cfg *Config, domain string, client *http.Client) {
	...
	for i, src := range allSources {
		...
		urls, err := src.Discover(ctx, domain, client)
		if err != nil {
	...
	results := extractAll(ctx, filteredURLs, client, cfg.Workers, bar)
	bar.Finish()
...
func extractAll(ctx context.Context, urls []string, client *http.Client, workers int, bar ProgressBar) []MetadataResult {
	...
	for i, u := range urls {
		...
		go func(idx int, fileURL string) {
			...
			results[idx] = extractMetadata(fileURL, client)
			bar.Add(1)
		}(i, u)
	}
