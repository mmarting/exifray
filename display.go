...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, "  -u, --urls string         File with list of URLs (skip discovery)\n")
	...
	fmt.Fprintf(os.Stderr, "      --max-retries int     Max retries on timeout (default 2)\n")
	fmt.Fprintf(os.Stderr, "      --retry-delay int     Retry delay in seconds (default 2)\n")
	...
	fmt.Fprintf(os.Stderr, "  exifray --urls file_urls.txt --show-urls\n")
	fmt.Fprintf(os.Stderr, "  exifray -d example.com --max-retries 3 --timeout 30\n")
